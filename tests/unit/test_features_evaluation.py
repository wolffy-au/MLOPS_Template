...
def test_keep_features_pandas():
    ...
    result_df = keep_features(df, features=[0, 1], keep_y=True)
...
def test_keep_features_numpy():
    ...
    np_array = np.array([[1, 4, 7], [2, 5, 8], [3, 6, 9]])
    
    # Test keeping features and target column
    result_np = keep_features(np_array, features=[0, 1], keep_y=True)
    expected_result = np.array([[1, 4, 7], [2, 5, 8], [3, 6, 9]])
    assert np.array_equal(result_np, expected_result)

    # Test keeping only specific features
    result_np = keep_features(np_array, features=[0], keep_y=False)
